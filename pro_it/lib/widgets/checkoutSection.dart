...
class CheckoutSection extends StatelessWidget {
  ...
  const CheckoutSection({Key? key, required this.price, required this.onCheckout}) : super(key: key);
  @override
              ...
              child: Row(
                children: <Widget>[
                  ...
                  GetX<CurrencyController>(
                    builder: (controller) {
                      var convPrice = controller.rate.value *
                              (price);
                          var currency = controller.currSymbol.value;
                      return Text(
                        "$currency.${convPrice.toStringAsFixed(2)}",
                        style: TextStyle(
